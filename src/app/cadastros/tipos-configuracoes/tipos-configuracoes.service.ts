...
export class TiposConfiguracoesService {
  ...
  public async searchPaginated(pageNumber: number, rownpPage: number, id?: number, descricao?: string, ativo?: boolean) {
    ...
    this.gridService.model = 'TiposConfiguracoes';
    ...
    this.service.getAll('cadastros_url', 'tiposconfiguracoes/paginated', this.parameters)
    .toPromise()
    .then(c => {
      if (c.length > 0) {
        this.gridService.gridBind.gridObjectBinded = c;
        this.gridService.addGridTitles(['#', 'Descrição', 'Ativo'], ['tipoConfiguracaoId', 'descricao', 'ativo']);
        let actions: ColunmAction[] = [];
        c.map(element => {
          actions.push(this.gridService
            .makeActionGridLine('edit', element.tipoConfiguracaoId, false, 'far fa-edit', '/cadastros/teste/cadastro/', element.descricao));
          actions.push(this.gridService.makeActionGridLine('delete', element.tipoConfiguracaoId, !element.ativo, 'far fa-trash-alt', '/cadastros/teste/deletar/', element.descricao));
          this.gridService.addGridLine([element.tipoConfiguracaoId, element.descricao, element.ativo.toString()], actions);
          actions = [];
        });
