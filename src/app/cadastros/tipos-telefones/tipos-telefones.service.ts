...
export class TiposTelefonesService {
  ...
  public async searchPaginated(pageNumber: number, rownpPage: number, id?: number, descricao?: string, ativo?: boolean) {
    ...
    this.gridService.model = 'TiposTelefones';
    ...
    this.service.getAll('cadastros_url', 'tipostelefones/paginated', this.parameters)
    .toPromise()
    .then(c => {
      if (c.length > 0) {
        this.gridService.gridBind.gridObjectBinded = c;
        this.gridService.addGridTitles(['#', 'Descrição', 'Ativo'], ['tipoTelefoneId', 'descricao', 'ativo']);
        let actions: ColunmAction[] = [];
        c.map(element => {
          actions.push(this.gridService
            .makeActionGridLine('edit', element.tipoTelefoneId, false
            , 'far fa-edit', '/cadastros/tipostelefones/cadastro/', element.descricao));
          actions.push(this.gridService.makeActionGridLine('delete', element.tipoTelefoneId, !element.ativo,
          'far fa-trash-alt', '/cadastros/tipostelefones/deletar/', element.descricao));
          this.gridService.addGridLine([element.tipoTelefoneId, element.descricao, element.ativo.toString()], actions);
          actions = [];
        });
      ...
      this.gridService.routeRegister = '/cadastros/tipostelefones/cadastro/';
    })
    .catch(e => {
      this.loaderService.SetLoaderState(false);
