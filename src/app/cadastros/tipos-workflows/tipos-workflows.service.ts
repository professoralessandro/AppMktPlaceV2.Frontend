...
export class TiposWorkflowsService {
  ...
  public async searchPaginated(pageNumber: number, rownpPage: number, id?: number, descricao?: string, ativo?: boolean) {
    ...
    this.gridService.model = 'TiposWorkflows';
    ...
    this.service.getAll('cadastros_url', 'tiposworkflows/paginated', this.parameters)
    .toPromise()
    .then(c => {
      if (c.length > 0) {
        this.gridService.gridBind.gridObjectBinded = c;
        this.gridService.addGridTitles(['#', 'Descrição', 'Ativo'], ['tipoWorkFlowId', 'descricao', 'ativo']);
        let actions: ColunmAction[] = [];
        c.map(element => {
          actions.push(this.gridService
            .makeActionGridLine('edit', element.tipoWorkFlowId, false
            , 'far fa-edit', '/cadastros/tiposworkflows/cadastro/', element.descricao));
          actions.push(this.gridService.makeActionGridLine('delete', element.tipoWorkFlowId, !element.ativo,
          'far fa-trash-alt', '/cadastros/tiposworkflows/deletar/', element.descricao));
          this.gridService.addGridLine([element.tipoWorkFlowId, element.descricao, element.ativo.toString()], actions);
          actions = [];
        });
      ...
      this.gridService.routeRegister = '/cadastros/tiposworkflows/cadastro/';
    })
    .catch(e => {
      this.loaderService.SetLoaderState(false);
